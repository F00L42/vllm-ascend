...
#include "ascend_mega_gdn_mtp_decode_pto_kernel.h"

// The included PTO implementation owns its explicit MTE2/V/MTE3 event chain.
struct AscendMegaGdnMtpDecodeTilingData {
  int64_t batch_size;
...
template <int32_t SpeculativeTokens,
          ...
          bool FlaSsmStateLayout = true>
#endif
AICORE PTO_INLINE void RunAscendMegaGdnMtpDecode(
    GM_ADDR qkv,
    GM_ADDR z,
    GM_ADDR b,
    GM_ADDR a,
...
#if defined(PTO_NPU_ARCH_A5)
  ascend_mega_gdn_mtp_decode_pto::Run<
      SpeculativeTokens,
      UseQkGroupCache,
      UseDeferredNorm,
      UseTwoOwnerQkGroups,
...
extern "C" __global__ __aicore__ void ascend_mega_gdn_mtp_decode(
    GM_ADDR qkv,
    GM_ADDR z,
    GM_ADDR b,
    GM_ADDR a,
  ...
  KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_MIX_AIC_1_2);
#endif
  REGISTER_TILING_DEFAULT(AscendMegaGdnMtpDecodeTilingData);
  GET_TILING_DATA_WITH_STRUCT(AscendMegaGdnMtpDecodeTilingData, tiling_data, tiling);
  (void)workspace;
